...
inline v2f InitializeV2F(appdata_full v, float4 projectedVertex, float isOutline)
{
    ...
    // Workaround for Metal on iOS 17 or later.
    worldNormal = clamp(worldNormal, -1.0, 1.0);
    worldTangent = clamp(worldTangent, -1.0, 1.0);
    worldBitangent = clamp(worldBitangent, -1.0, 1.0);
#endif
